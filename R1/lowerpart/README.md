...
| 执行器 | 数量 | CAN ID | 控制总线 | 电机类型 | 状态 |
|--------|------|--------|----------|----------|------|
| 底盘电机 (BR) | 1 | 0x01 | CAN1 | DM4310 MIT 速度模式 | ✅ |
| 底盘电机 (FR) | 1 | 0x02 | CAN1 | DM4310 MIT 速度模式 | ✅ |
| 底盘电机 (BL) | 1 | 0x04 | CAN1 | DM4310 MIT 速度模式 | ✅ |
| 底盘电机 (FL) | 1 | 0x03 | CAN1 | DM4310 MIT 速度模式 | ✅ |
| XT 输出 | 4 | — | PH2~PH5 | GPIO 直驱 | ✅ |

> 底盘 489.28×489.28mm, 全向轮径 127mm, CHASSIS_R=0.346m, 轮询顺序 BR→FR→BL→FL, MIT kp=0 + kd=CHASSIS_TORQUE
...
| `chassis_task` | ~2ms | 512 | 0 | 45° 全向轮运动学 + MIT 速度控制 |
...
- FreeRTOS 多任务框架 + CAN 滤波器配置
- **底盘运动学**: 45° 四全向轮逆运动学 (MIT+kp=0 速度控制)
- **电机使能/失能**: CH4 边沿触发, 上升沿使能 + 下降沿失能
- **CAN 反馈**: CAN1 `dm_rx_cbk` 解析 DM4310 位置/速度/扭矩/温度
- LED 流水灯
